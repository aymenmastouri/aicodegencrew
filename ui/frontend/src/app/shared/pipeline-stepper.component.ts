...
@Component({
      ...
      @for (step of steps; track step.phase_id; let i = $index; let last = $last) {
        <div class="stepper-step" [class]="'step-' + step.status">
          ...
          @if (step.status === 'running') {
            <div class="step-time step-time-active">{{ step.duration_seconds ? fmt(step.duration_seconds) : 'running' }}</div>
          }
  ...
  styles: [`
    ...
    .step-num { font-size: 13px; font-weight: 600; color: var(--cg-gray-400); }
    /* Running */
    ...
    /* Completed */
    .step-completed .step-circle { background: var(--cg-success, #28a745); border-color: var(--cg-success, #28a745); }
    .step-check { font-size: 18px; width: 18px; height: 18px; color: #fff; }
    /* Partial */
    .step-partial .step-circle { background: var(--cg-warn, #f57c00); border-color: var(--cg-warn, #f57c00); }
    .step-warn { font-size: 18px; width: 18px; height: 18px; color: #fff; }
    /* Failed */
    .step-failed .step-circle { background: var(--cg-error, #dc3545); border-color: var(--cg-error, #dc3545); }
    .step-fail { font-size: 18px; width: 18px; height: 18px; color: #fff; }
    /* Cancelled */
    .step-cancelled .step-circle { background: var(--cg-warn, #f57c00); border-color: var(--cg-warn, #f57c00); }
    ...
    /* Skipped */
    .step-skipped .step-circle { background: var(--cg-success, #28a745); border-color: var(--cg-success, #28a745); opacity: 0.7; }
    ...
    @media (prefers-reduced-motion: reduce) {
      .step-running .step-circle { animation-duration: 0.01ms !important; }
      .line-active::after { animation-duration: 0.01ms !important; }
    }
  `],
})
