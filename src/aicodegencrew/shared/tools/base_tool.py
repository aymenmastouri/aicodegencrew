...
class EnhancedBaseTool(BaseTool, ABC):
    ...
    def _execute_with_retry(self, func, *args, **kwargs) -> Dict[str, Any]:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                last_error = e
                logger.warning(
                    f"Tool {self.name} attempt {attempt + 1}/{self.max_retries} failed: {str(e)}"
                )
        ...
        error_msg = f"Tool {self.name} failed after {self.max_retries} attempts: {str(last_error)}"
    ...
    def _log_metrics(self, metrics: ToolMetrics):
        ...
        if metrics.success:
            ...
            logger.error(
                f"[FAIL] {metrics.tool_name} failed after {metrics.retry_count} attempts: "
