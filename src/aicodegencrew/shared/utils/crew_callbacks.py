...
def step_callback(step_output) -> None:
    ...
    try:
        ...
        if thought:
            ...
            logger.debug(f"[THINK] {agent_name}: {thought_str[:500]}{'...' if len(thought_str) > 500 else ''}")
        ...
        if tool:
            ...
            logger.info(f"[TOOL] {tool}: {input_str[:200]}{'...' if len(input_str) > 200 else ''}")

        # Tool result — verbose, file only
        result = getattr(step_output, "result", None) or getattr(step_output, "observation", None)
        if result:
            result_str = str(result)
            logger.debug(f"[TOOL_RESULT] {result_str[:300]}{'...' if len(result_str) > 300 else ''}")
...
def task_callback(task_output) -> None:
    ...
    try:
        ...
        if raw:
            output_str = str(raw)
            logger.debug(f"[TASK_OUTPUT] {output_str[:500]}{'...' if len(output_str) > 500 else ''}")
