...
class ArchitectureFactsValidator:
    ...
    def _validate_schema(self):
        ...
        for field in required_fields:
            ...
            elif not isinstance(self.facts.get(field), list):
                self.errors.append(ValidationError(
                    severity="error",
                    category="schema",
        ...
        for i, container in enumerate(containers):
            ...
            if not container.get("id"):
                self.errors.append(ValidationError(
                    severity="error",
                    category="schema",
                    message="Container missing 'id' field",
                    location=loc
                ))
            else:
        ...
        for i, iface in enumerate(interfaces):
            ...
            if not iface.get("id"):
                self.errors.append(ValidationError(
                    severity="error",
                    category="schema",
                    message="Interface missing 'id' field",
                    location=loc
                ))
        ...
        for i, rel in enumerate(relations):
            ...
            if not from_id:
                self.errors.append(ValidationError(
                    severity="error",
                    category="schema",
                    message="Relation missing 'from' field",
                    location=loc
                ))
            ...
            if not to_id:
                self.errors.append(ValidationError(
                    severity="error",
                    category="schema",
                    message="Relation missing 'to' field",
                    location=loc
                ))
...
if __name__ == "__main__":
    ...
    print(f"\n{'='*60}")
    print(f"ARCHITECTURE FACTS VALIDATION REPORT")
    print(f"{'='*60}")
