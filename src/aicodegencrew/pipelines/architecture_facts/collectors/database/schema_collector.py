...
class OracleSchemaCollector(DimensionCollector):
    ...
    def _extract_schemas(self, file_path: Path, content: str) -> List[RawEntity]:
        ...
        schema_matches = re.finditer(
            r'CREATE\s+SCHEMA\s+(?:AUTHORIZATION\s+)?(\w+)',
            content,
            re.IGNORECASE
        )
            ...
            if schema_name not in seen_schemas:
                ...
                facts.append(RawEntity(
                    ...
                    evidence=RawEvidence(
                        ...
                        line_start=line_num,
                        line_end=line_num + 1,
                        reason=f"Schema reference in qualified name",
                    ),
                ))
    ...
    def _extract_tablespaces(self, file_path: Path, content: str) -> List[RawEntity]:
        ...
        # CREATE TABLESPACE
        ts_matches = re.finditer(
            r'CREATE\s+(?:BIGFILE\s+)?TABLESPACE\s+(\w+)',
            content,
            re.IGNORECASE
        )
    ...
    def _extract_synonyms(self, file_path: Path, content: str) -> List[RawEntity]:
        ...
        syn_matches = re.finditer(
            r'CREATE\s+(?:OR\s+REPLACE\s+)?(?:PUBLIC\s+)?SYNONYM\s+(\w+)\s+FOR\s+(\S+)',
            content,
            re.IGNORECASE
        )
    ...
    def _extract_db_links(self, file_path: Path, content: str) -> List[RawEntity]:
        ...
        link_matches = re.finditer(
            r'CREATE\s+(?:SHARED\s+)?(?:PUBLIC\s+)?DATABASE\s+LINK\s+(\w+)',
            content,
            re.IGNORECASE
        )
