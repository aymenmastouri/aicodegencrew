...
class OracleTableCollector(DimensionCollector):
    ...
    CREATE_TABLE_PATTERN = re.compile(
        r"CREATE\s+TABLE\s+(?:IF\s+NOT\s+EXISTS\s+)?(?:(\w+)\.)?(\w+)\s*\(", re.IGNORECASE | re.DOTALL
    )
    ...
    PRIMARY_KEY_PATTERN = re.compile(r"(?:CONSTRAINT\s+(\w+)\s+)?PRIMARY\s+KEY\s*\(([^)]+)\)", re.IGNORECASE)
    ...
    CREATE_INDEX_PATTERN = re.compile(
        r"CREATE\s+(?:UNIQUE\s+)?INDEX\s+(\w+)\s+ON\s+(?:(\w+)\.)?(\w+)\s*\(([^)]+)\)", re.IGNORECASE
    )

    CREATE_VIEW_PATTERN = re.compile(r"CREATE\s+(?:OR\s+REPLACE\s+)?VIEW\s+(?:(\w+)\.)?(\w+)\s+AS", re.IGNORECASE)
    ...
    def _process_sql_file(self, file_path: Path):
        ...
        for match in self.CREATE_VIEW_PATTERN.finditer(content):
            ...
            view.add_evidence(
                path=rel_path, line_start=line_num, line_end=line_num + 5, reason=f"CREATE VIEW: {view_name}"
            )
