"""
Collector Registry — Single source of truth for all collector metadata.

Used by:
- CollectorOrchestrator (to know step ordering and IDs)
- Dashboard backend (to list collectors with metadata)
- Collector config (to validate IDs)
"""

from __future__ import annotations

# Ecosystem IDs for reference:
# "java_jvm", "javascript_typescript", "c_cpp", "python"
# Empty list = cross-cutting (runs always, independent of ecosystem)

COLLECTOR_REGISTRY: list[dict] = [
    {
        "id": "system",
        "name": "System Facts",
        "description": "System name, version, subsystems from pom.xml/package.json",
        "dimension": "system",
        "category": "core",
        "collector_type": "structure",
        "step": 1,
        "output_file": "system.json",
        "class_name": "SystemCollector",
        "can_disable": False,
        "ecosystems": [],  # cross-cutting
    },
    {
        "id": "containers",
        "name": "Container Detection",
        "description": "Deployable units (Spring Boot, Angular, databases, Docker)",
        "dimension": "containers",
        "category": "core",
        "collector_type": "structure",
        "step": 2,
        "output_file": "containers.json",
        "class_name": "ContainerCollector",
        "can_disable": False,
        "ecosystems": ["java_jvm", "javascript_typescript", "c_cpp", "python"],
    },
    {
        "id": "components",
        "name": "Component Extraction",
        "description": "Controllers, services, repositories + Spring/Angular specialists",
        "dimension": "components",
        "category": "core",
        "collector_type": "structure",
        "step": 3,
        "output_file": "components.json",
        "class_name": "ComponentCollector",
        "can_disable": False,
        "ecosystems": ["java_jvm", "javascript_typescript", "c_cpp", "python"],
    },
    {
        "id": "interfaces",
        "name": "Interface Extraction",
        "description": "REST endpoints, Angular routes, message channels, gRPC services",
        "dimension": "interfaces",
        "category": "optional",
        "collector_type": "interface",
        "step": 4,
        "output_file": "interfaces.json",
        "class_name": "InterfaceCollector",
        "can_disable": True,
        "ecosystems": ["java_jvm", "javascript_typescript", "c_cpp", "python"],
    },
    {
        "id": "data_model",
        "name": "Data Model",
        "description": "JPA entities, SQLAlchemy/Django models, database tables, migrations",
        "dimension": "data_model",
        "category": "optional",
        "collector_type": "data",
        "step": 5,
        "output_file": "data_model.json",
        "class_name": "DataModelCollector",
        "can_disable": True,
        "ecosystems": ["java_jvm", "python"],
    },
    {
        "id": "runtime",
        "name": "Runtime Facts",
        "description": "Schedulers, async methods, event listeners, Celery tasks",
        "dimension": "runtime",
        "category": "optional",
        "collector_type": "runtime",
        "step": 6,
        "output_file": "runtime.json",
        "class_name": "RuntimeCollector",
        "can_disable": True,
        "ecosystems": ["java_jvm", "python"],
    },
    {
        "id": "infrastructure",
        "name": "Infrastructure",
        "description": "Docker, Kubernetes, CI/CD pipelines",
        "dimension": "infrastructure",
        "category": "optional",
        "collector_type": "infrastructure",
        "step": 7,
        "output_file": "infrastructure.json",
        "class_name": "InfrastructureCollector",
        "can_disable": True,
        "ecosystems": [],  # cross-cutting
    },
    {
        "id": "dependencies",
        "name": "Dependencies",
        "description": "External libraries and packages from pom.xml/package.json",
        "dimension": "dependencies",
        "category": "optional",
        "collector_type": "build",
        "step": 8,
        "output_file": "dependencies.json",
        "class_name": "DependencyCollector",
        "can_disable": True,
        "ecosystems": ["java_jvm", "javascript_typescript", "c_cpp", "python"],
    },
    {
        "id": "workflows",
        "name": "Workflows",
        "description": "State machines, BPMN processes, NgRx stores, Celery chains, Airflow DAGs",
        "dimension": "workflows",
        "category": "optional",
        "collector_type": "runtime",
        "step": 9,
        "output_file": "workflows.json",
        "class_name": "WorkflowCollector",
        "can_disable": True,
        "ecosystems": ["java_jvm", "javascript_typescript", "c_cpp", "python"],
    },
    {
        "id": "tech_versions",
        "name": "Technology Versions",
        "description": "Framework and library versions for upgrade planning",
        "dimension": "tech_versions",
        "category": "optional",
        "collector_type": "build",
        "step": 10,
        "output_file": "tech_versions.json",
        "class_name": "TechStackVersionCollector",
        "can_disable": True,
        "ecosystems": ["java_jvm", "javascript_typescript", "c_cpp", "python"],
    },
    {
        "id": "security_details",
        "name": "Security Details",
        "description": "Method-level security, CSRF, CORS, Django/Flask/FastAPI auth",
        "dimension": "security_details",
        "category": "optional",
        "collector_type": "quality",
        "step": 11,
        "output_file": "security_details.json",
        "class_name": "SecurityDetailCollector",
        "can_disable": True,
        "ecosystems": ["java_jvm", "javascript_typescript", "python"],
    },
    {
        "id": "validation",
        "name": "Validation Rules",
        "description": "Bean Validation, Pydantic, Marshmallow, Django forms, Angular validators",
        "dimension": "validation",
        "category": "optional",
        "collector_type": "quality",
        "step": 12,
        "output_file": "validation.json",
        "class_name": "ValidationCollector",
        "can_disable": True,
        "ecosystems": ["java_jvm", "javascript_typescript", "python"],
    },
    {
        "id": "tests",
        "name": "Tests",
        "description": "Unit, integration, e2e tests: JUnit, Cucumber, pytest, GoogleTest, Catch2",
        "dimension": "tests",
        "category": "optional",
        "collector_type": "quality",
        "step": 13,
        "output_file": "tests.json",
        "class_name": "TestCollector",
        "can_disable": True,
        "ecosystems": ["java_jvm", "javascript_typescript", "c_cpp", "python"],
    },
    {
        "id": "error_handling",
        "name": "Error Handling",
        "description": "Exception handlers, @ControllerAdvice, Flask errorhandler, custom exceptions",
        "dimension": "error_handling",
        "category": "optional",
        "collector_type": "quality",
        "step": 14,
        "output_file": "error_handling.json",
        "class_name": "ErrorHandlingCollector",
        "can_disable": True,
        "ecosystems": ["java_jvm", "javascript_typescript", "python"],
    },
    {
        "id": "build_system",
        "name": "Build System",
        "description": "Build tools, modules, tasks/scripts (Gradle, Maven, npm, CMake, Meson, pyproject.toml)",
        "dimension": "build_system",
        "category": "optional",
        "collector_type": "build",
        "step": 15,
        "output_file": "build_system.json",
        "class_name": "BuildSystemCollector",
        "can_disable": True,
        "ecosystems": ["java_jvm", "javascript_typescript", "c_cpp", "python"],
    },
    {
        "id": "configuration",
        "name": "Configuration",
        "description": "Config files, environment variables, profiles, feature flags",
        "dimension": "configuration",
        "category": "optional",
        "collector_type": "infrastructure",
        "step": 16,
        "output_file": "configuration.json",
        "class_name": "ConfigurationCollector",
        "can_disable": True,
        "ecosystems": ["java_jvm", "javascript_typescript", "c_cpp", "python"],
    },
    {
        "id": "logging_observability",
        "name": "Logging & Observability",
        "description": "Logging frameworks, tracing, metrics endpoints, health checks",
        "dimension": "logging_observability",
        "category": "optional",
        "collector_type": "quality",
        "step": 17,
        "output_file": "logging_observability.json",
        "class_name": "LoggingObservabilityCollector",
        "can_disable": True,
        "ecosystems": ["java_jvm", "javascript_typescript", "c_cpp", "python"],
    },
    {
        "id": "technical_debt",
        "name": "Technical Debt",
        "description": "TODO/FIXME/HACK comments, deprecated usage, suppressed warnings",
        "dimension": "technical_debt",
        "category": "optional",
        "collector_type": "quality",
        "step": 18,
        "output_file": "technical_debt.json",
        "class_name": "TechnicalDebtCollector",
        "can_disable": True,
        "ecosystems": [],  # cross-cutting
    },
    {
        "id": "api_contracts",
        "name": "API Contracts",
        "description": "OpenAPI/Swagger, gRPC proto, GraphQL schemas, AsyncAPI, WSDL",
        "dimension": "api_contracts",
        "category": "optional",
        "collector_type": "interface",
        "step": 19,
        "output_file": "api_contracts.json",
        "class_name": "ApiContractsCollector",
        "can_disable": True,
        "ecosystems": [],  # cross-cutting
    },
    {
        "id": "communication_patterns",
        "name": "Communication Patterns",
        "description": "Message queues, event buses, WebSockets, REST clients, pub/sub",
        "dimension": "communication_patterns",
        "category": "optional",
        "collector_type": "runtime",
        "step": 20,
        "output_file": "communication_patterns.json",
        "class_name": "CommunicationPatternsCollector",
        "can_disable": True,
        "ecosystems": ["java_jvm", "javascript_typescript", "c_cpp", "python"],
    },
    {
        "id": "evidence",
        "name": "Evidence Aggregation",
        "description": "Cross-references and source evidence from all collectors",
        "dimension": "evidence",
        "category": "core",
        "collector_type": "meta",
        "step": 21,
        "output_file": "evidence_map.json",
        "class_name": "EvidenceCollector",
        "can_disable": False,
        "ecosystems": [],  # cross-cutting
    },
]


def get_registry_by_id() -> dict[str, dict]:
    """Return registry as a dict keyed by collector ID."""
    return {c["id"]: c for c in COLLECTOR_REGISTRY}


def get_disableable_ids() -> list[str]:
    """Return list of collector IDs that can be disabled."""
    return [c["id"] for c in COLLECTOR_REGISTRY if c["can_disable"]]
