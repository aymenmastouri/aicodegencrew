...
from .base import DimensionCollector, CollectorOutput, RawFact, FactType
...
class WorkflowCollector(DimensionCollector):
    ...
    STATUS_ENUM_PATTERN = re.compile(r'enum\s+(Enum\w*(?:Status|State)\w*|(?:Status|State)\w*Enum|\w+Status|\w+State)\s*\{', re.IGNORECASE)
    ...
    def _parse_bpmn_file(self, file_path: Path):
        ...
        try:
            ...
            for process in processes:
                ...
                states = []
                for elem_type in ['userTask', 'serviceTask', 'scriptTask', 'manualTask', 'businessRuleTask', 'startEvent', 'endEvent']:
    ...
    def _check_spring_statemachine(self, file_path: Path, content: str):
        ...
        workflow.add_evidence(
            ...
            line_start=1,
            line_end=50,
            reason=f"Spring State Machine: {class_name}"
        )
    ...
    def _check_custom_statemachine(self, file_path: Path, content: str):
        ...
        logger.debug(f"[WorkflowCollector] Found Custom StateMachine: {class_name} ({len(states)} states, {len(actions)} actions)")
