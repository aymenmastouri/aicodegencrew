...
from ..base import CollectorOutput, DimensionCollector, RawInterface, RelationHint
...
class AngularRoutingCollector(DimensionCollector):
    ...
    def _process_routing_file(self, file_path: Path):
        ...
        for match in self.ROUTE_PATTERN.finditer(content):
            ...
            route.add_evidence(
                path=rel_path, line_start=line_num, line_end=line_num + 5, reason=f"Angular route: /{path}"
            )
