...
class AngularServiceCollector(DimensionCollector):
    ...
    OPENAPI_PATH_PATTERN = re.compile(
        r'static\s+readonly\s+(\w+Path)\s*=\s*[\'"]([^\'"]+)[\'"]'
    )
    ...
    JS_API_ENDPOINT_PATTERN = re.compile(
        r'static\s+ENDPOINT\s*=\s*[\'"](\w+)[\'"]'
    )
    ...
    def _process_service_file(self, file_path: Path):
        ...
        service.add_evidence(
            path=rel_path,
            line_start=class_line - 3,
            line_end=class_line + 3,
            reason=f"@Injectable: {service_name}"
        )
        ...
        for call in http_calls:
            if call.get("url", "").startswith("/api"):
                ...
                relation.evidence.append(self._create_evidence(
                    file_path, 1, 1,
                    f"HTTP {call.get('method', 'GET')} call to {call.get('url', '')}"
                ))
                self.output.add_relation(relation)
    ...
    def _extract_dependencies(self, content: str) -> List[str]:
        ...
        if constructor_match:
            ...
            for param_match in self.PARAM_PATTERN.finditer(constructor_content):
                ...
                if dep_type not in ('HttpClient', 'Router', 'ActivatedRoute', 
                                    'FormBuilder', 'NgZone', 'Renderer2'):
