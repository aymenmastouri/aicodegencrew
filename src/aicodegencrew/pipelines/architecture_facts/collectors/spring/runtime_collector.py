"""Spring Runtime Specialist — Extracts Java/Kotlin runtime behavior facts.

Detects:
- @Scheduled annotations (cron, fixedRate, fixedDelay)
- @Async methods
- @EventListener handlers
- Spring Batch job and step definitions
"""
...
class SpringRuntimeCollector(DimensionCollector):
    """Extracts Spring/Java runtime behavior facts."""
    ...
    def collect(self) -> CollectorOutput:
        """Collect runtime behavior facts from Java/Kotlin files."""
        self._log_start()
        ...
        for kt_file in kotlin_files:
            self._process_java_file(kt_file)

        self._log_end()
        return self.output

    def _process_java_file(self, file_path: Path):
        """Process a Java/Kotlin file for runtime facts."""
    ...
    def _extract_scheduler(self, match, content: str, lines: list[str], file_path: str):
        ...
        fact.add_evidence(
            path=file_path, line_start=line_num, line_end=line_num + 5, reason=f"@Scheduled: {method_name}"
        )
    ...
    def _extract_async(self, match, content: str, lines: list[str], file_path: str):
        ...
        fact = RawRuntimeFact(name=method_name, type="async", metadata={"async": True})

        fact.add_evidence(path=file_path, line_start=line_num, line_end=line_num + 3, reason=f"@Async: {method_name}")
    ...
    def _extract_batch_jobs(self, content: str, lines: list[str], file_path: str):
        ...
        for match in job_pattern.finditer(content):
            ...
            fact.add_evidence(
                path=file_path, line_start=line_num, line_end=line_num + 10, reason=f"Spring Batch Job: {job_name}"
            )
        ...
        for match in step_pattern.finditer(content):
            ...
            fact.add_evidence(
                path=file_path, line_start=line_num, line_end=line_num + 5, reason=f"Spring Batch Step: {step_name}"
            )
