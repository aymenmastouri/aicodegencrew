...
class SpringRestCollector(DimensionCollector):
    ...
    REST_SERVICE_PATTERN = re.compile(
        r'(?:class|interface)\s+(\w*RestService\w*)',
        re.MULTILINE
    )
    ...
    def _extract_rest_service_endpoints(self, content: str, lines: List[str],
                                         file_path: str, class_name: str, base_path: str):
        """
        ...
        HTTP_METHOD_PREFIXES = {
            'get': 'GET', 'find': 'GET', 'fetch': 'GET', 'load': 'GET', 'read': 'GET', 'list': 'GET', 'search': 'GET',
            'create': 'POST', 'add': 'POST', 'save': 'POST', 'insert': 'POST', 'register': 'POST',
    ...
    def _derive_path_from_method(self, method_name: str) -> str:
        ...
        prefixes = ['get', 'find', 'fetch', 'load', 'read', 'list', 'search',
                    'create', 'add', 'save', 'insert', 'register',
                    'update', 'modify', 'edit', 'change',
                    'delete', 'remove', 'destroy', 'patch']
