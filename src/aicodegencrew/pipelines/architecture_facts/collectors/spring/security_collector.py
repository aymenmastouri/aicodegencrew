...
class SpringSecurityCollector(DimensionCollector):
    ...
    def collect(self) -> CollectorOutput:
        ...
        for props_file in list(self.repo_path.rglob("application*.yml")) + list(self.repo_path.rglob("application*.yaml")) + list(self.repo_path.rglob("application*.properties")):
            try:
    ...
    def _extract_auth_providers(self, file_path: Path, content: str) -> List[RawComponent]:
        ...
        provider_match = re.search(
            r'class\s+(\w+)\s+implements\s+.*AuthenticationProvider',
            content
        )
        if provider_match:
        ...
        uds_match = re.search(
            r'class\s+(\w+)\s+implements\s+.*UserDetailsService',
            content
        )
        if uds_match:
    ...
    def _extract_filters(self, file_path: Path, content: str) -> List[RawComponent]:
        ...
        filter_match = re.search(
            r'class\s+(\w+)\s+extends\s+OncePerRequestFilter',
            content
        )
        if filter_match:
