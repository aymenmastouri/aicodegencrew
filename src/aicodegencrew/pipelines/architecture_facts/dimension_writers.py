...
from .model_builder import ArchitectureModel, CanonicalComponent, ArchitectureLayer
...
class InterfacesWriter(DimensionWriterBase):
    ...
    def write(self, model: ArchitectureModel) -> Path:
        ...
        for iface in model.interfaces.values():
            ...
            if iface.method:
                by_type[iface_type]["methods"][iface.method] = \
                    by_type[iface_type]["methods"].get(iface.method, 0) + 1
...
class DataModelWriter(DimensionWriterBase):
    ...
    def write(self, model: ArchitectureModel) -> Path:
        ...
        repositories = model.get_components_by_stereotype("repository")
...
class RuntimeWriter(DimensionWriterBase):
    ...
    def write(self, model: ArchitectureModel, endpoint_flows: List[Dict] = None) -> Path:
        ...
        schedulers = [
            c for c in model.components.values()
            if "scheduler" in c.stereotype.lower() or "scheduled" in c.name.lower()
        ]
        
        # Get async components
        async_components = [
            c for c in model.components.values()
            if "async" in c.stereotype.lower() or "async" in c.name.lower()
        ]
...
class InfrastructureWriter(DimensionWriterBase):
    ...
    def write(self, model: ArchitectureModel) -> Path:
        ...
        infra_stereotypes = {"dockerfile", "compose_service", "k8s_deployment", "k8s_service", 
                           "deployment", "k8s-service", "ingress", "configmap", "helm-chart",
                           "ci_pipeline", "gitlab-ci", "github-actions", "jenkins"}
        for comp in model.components.values():
        ...
        kubernetes = [c for c in infra_components if c.stereotype in ("deployment", "k8s-service", "k8s_deployment", "k8s_service", "ingress", "configmap", "helm-chart")]
        ci_cd = [c for c in infra_components if c.stereotype in ("ci_pipeline", "gitlab-ci", "github-actions", "jenkins", "azure-devops", "circleci")]
        config = [c for c in infra_components if c.stereotype == "configuration"]
        
        data = {
            "docker": {
                "total": len(docker),
                "items": [{"id": c.id, "name": c.name, "file_path": c.file_paths[0] if c.file_paths else None, "stereotype": c.stereotype} for c in docker],
            },
            "kubernetes": {
