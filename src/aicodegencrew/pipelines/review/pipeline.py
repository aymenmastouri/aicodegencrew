"""ReviewPipeline — consistency guard + LLM synthesis.
...
class ReviewPipeline(BasePipeline):
    ...
    Direct usage::
        ...
        crew = ReviewPipeline(knowledge_dir="knowledge")
        result = crew.run()
    """
    ...
    def run(self) -> dict[str, Any]:
        ...
        try:
            ...
            logger.error("[ReviewPipeline] LLM synthesis failed: %s", e)
        ...
        logger.info(
            "[ReviewPipeline] Done — score=%d, issues=%d, duration=%.1fs",
    ...
    def _load_fact_containers(self) -> list[str]:
        ...
        if not path.exists():
            logger.warning("[ReviewPipeline] containers.json not found: %s", path)
            return []
        try:
            ...
            logger.error("[ReviewPipeline] Failed loading containers.json: %s", e)
            return []
    ...
    def _scan_c4_for_containers(self, container_names: list[str]) -> list[str]:
        ...
        try:
            ...
            logger.error("[ReviewPipeline] Error scanning C4 diagrams: %s", e)
            return []
...
"""
        ...
        logger.info("[ReviewLLM] Calling LLMGenerator")
        ...
        try:
            ...
            logger.info("[ReviewPipeline] Synthesis report written: %s", output_path)
            return True
        except Exception as e:
            logger.error("[ReviewPipeline] LLM synthesis failed: %s", e)
            return False
    ...
    def _write_json(self, data: dict, filename: str) -> None:
        ...
        try:
            ...
            logger.error("[ReviewPipeline] Failed writing %s: %s", filename, e)
