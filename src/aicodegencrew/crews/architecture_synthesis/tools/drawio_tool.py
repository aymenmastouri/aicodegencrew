...
class DrawioDiagramInput(BaseModel):
    ...
    file_path: str = Field(
        description="The path to save the .drawio file (relative to knowledge/architecture/)"
    )
    diagram_name: str = Field(
        description="Name of the diagram (shown in tab)"
    )
    ...
    def _run(self, file_path: str, diagram_name: str, nodes: List[Dict[str, Any]], edges: List[Dict[str, Any]]) -> str:
        ...
        try:
            ...
            for node in nodes:
                ...
                xml_parts.append(
                    f'    <mxCell id="{node_id}" value="{label}" style="{style}" vertex="1" parent="1">'
                )
                xml_parts.append(
