...
LAYER_PRIORITY = {
    ...
    "page": 1,
    "container": 1,
    "module": 2,  # Shared services (also backend)
...
class StereotypeListInput(BaseModel):
    ...
    limit: int = Field(default=100, description="Maximum results to return (default 100, max 500 for large repos)")
    offset: int = Field(default=0, description="Number of results to skip for pagination (default 0)")
...
class StereotypeListTool(BaseTool):
    ...
    def _run(self, stereotype: str, container: str = "", limit: int = 100, offset: int = 0) -> str:
        ...
        result = {
            ...
            "note": f"Showing {start_idx + 1}-{end_idx} of {total_count}. Use offset={end_idx} to get next page."
            if end_idx < total_count
            else f"Showing all {total_count} results.",
            "components": [
                {
