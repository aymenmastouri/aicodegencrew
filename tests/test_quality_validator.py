...
def test_valid_facts_file(temp_dir):
    ...
    facts = {
        "containers": [
            {"id": "backend", "name": "Backend", "technology": "Spring Boot"}
        ],
        "components": [
            {"id": "user_service", "name": "UserService", "container": "backend", "stereotype": "service", "evidence_ids": ["ev_001"]}
        ],
        "interfaces": [
            {"id": "if_001", "type": "REST", "path": "/api/users", "implemented_by": "user_service", "evidence_ids": ["ev_001"]}
        ],
        "relations": [
            {"from": "user_service", "to": "user_service", "type": "uses"}
        ],
        "evidence": [
            {"id": "ev_001", "path": "src/UserService.java", "lines": "1-10", "reason": "Service annotation"}
        ]
    }
...
def test_invalid_evidence_reference(temp_dir):
    ...
    facts = {
        "containers": [{"id": "backend", "name": "Backend"}],
        "components": [
            {"id": "service_a", "name": "ServiceA", "container": "backend", "evidence_ids": ["ev_999"]}  # Invalid evidence
        ],
        "interfaces": [],
        "relations": [],
        "evidence": [
            {"id": "ev_001", "path": "src/test.java", "lines": "1-10"}
        ]
    }
